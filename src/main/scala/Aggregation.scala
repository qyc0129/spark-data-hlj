...
//  val lines2=lines.map((f:String)=>split.split(f)).filter((m:String)=>m!="")//用细化过的split函数进行筛选
...
//    val output2=lines4.saveAsTextFile(path2)//储存筛选后的话单数据
  ...
  val df3=df2.join(df1,Seq("lac","ci"))//两表按"lac"与"ci"两键值进行内链接
    df3.write.format("csv").save(path3)//储存最终结果
  }
